...
export default function LogoConceptGenerator() {
  ...
  const [previewResult, setPreviewResult] = useState<Concept | null>(null);
  const [fullResult, setFullResult] = useState<Concept[] | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/logo-concept", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ businessName, industry, style, values }),
      });
      const data = await res.json();
      if (isPro) {
        setFullResult(data.concepts);
        setPreviewResult(null);
      } else {
        setPreviewResult(data.concepts[0]);
        setFullResult(null);
        setShowPaywall(true);
      }
    } catch (err) {
    ...
    <main style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ maxWidth: "700px", margin: "0 auto", padding: "60px 40px 40px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "42px", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-1px", marginBottom: "16px" }}>
          Logo Concept <span style={{ color: "#7c3aed" }}>Generator</span>
        </h1>
        <p style={{ fontSize: "16px", color: "#888", lineHeight: 1.6, marginBottom: "40px", fontWeight: 300 }}>
          Generate creative logo concepts and direction for your designer.
        </p>
      </section>
      ...
      <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
          ...
          <input value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="Industry (e.g. tech, fitness, food...)" style={inputStyle} />
          ...
          <input value={values} onChange={(e) => setValues(e.target.value)} placeholder="Brand values (e.g. innovation, trust, energy...)" style={inputStyle} />
        </div>
        <button onClick={handleGenerate} disabled={loading || !businessName.trim()} style={{
          ...
          {loading ? "Generating..." : "Generate Logo Concepts"}
        </button>
      </section>
      ...
      {previewResult && !isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 20px" }}>
          <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
            <div style={{ fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", color: "#444", marginBottom: "10px" }}>Preview — Concept 1</div>
            <div style={{ fontSize: "18px", fontWeight: 800, marginBottom: "12px" }}>{previewResult.name}</div>
            <div style={{ fontSize: "13px", color: "#888", marginBottom: "16px", lineHeight: 1.6 }}>{previewResult.description}</div>
            <div style={{ display: "flex", gap: "8px" }}>
              {previewResult.colors?.map((color: string, i: number) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <div style={{ width: "24px", height: "24px", borderRadius: "50%", background: color, border: "1px solid #333" }} />
                  <span style={{ fontSize: "11px", color: "#555" }}>{previewResult.colorNames?.[i]}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
          <div style={{ position: "relative" }}>
            <div style={{ filter: "blur(6px)", pointerEvents: "none", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["Concept 2 — Alternative Direction", "Concept 3 — Bold Variation"].map((t, i) => (
                <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "10px", padding: "16px 20px" }}>
                  ...
                  <div style={{ fontSize: "12px", color: "#555", marginTop: "4px" }}>Full concept with colors, typography and symbolism.</div>
                </div>
              ))}
            </div>
            <div style={{
              ...
              <div style={{
                ...
                <h3 style={{ fontSize: "20px", fontWeight: 800, marginBottom: "8px" }}>Unlock all 3 logo concepts</h3>
      ...
      {fullResult && isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 80px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            {fullResult.map((concept, i) => (
              <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
                <div style={{ fontSize: "11px", color: "#444", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "8px" }}>Concept {i + 1}</div>
                <div style={{ fontSize: "18px", fontWeight: 800, marginBottom: "10px" }}>{concept.name}</div>
                <div style={{ fontSize: "13px", color: "#888", marginBottom: "12px", lineHeight: 1.6 }}>{concept.description}</div>
                <div style={{ fontSize: "12px", color: "#666", marginBottom: "12px", paddingLeft: "12px", borderLeft: "2px solid #333" }}>{concept.symbolism}</div>
                <div style={{ display: "flex", gap: "10px", marginBottom: "12px", flexWrap: "wrap" }}>
                  {concept.colors?.map((color: string, j: number) => (
                    <div key={j} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <div style={{ width: "20px", height: "20px", borderRadius: "50%", background: color, border: "1px solid #333" }} />
                      <span style={{ fontSize: "11px", color: "#555" }}>{concept.colorNames?.[j]}</span>
                    </div>
                  ))}
                </div>
                <div style={{ fontSize: "12px", color: "#555" }}>Typography: {concept.typography}</div>
                <div style={{ fontSize: "12px", color: "#a78bfa", marginTop: "4px" }}>Mood: {concept.mood}</div>
              </div>
            ))}
          </div>
        </section>
