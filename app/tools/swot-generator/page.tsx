...
export default function SWOTGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/swot", {
        ...
        body: JSON.stringify({ businessName, industry, competitors, goals }),
      });
  ...
  const quadrant = (label: string, items: string[], color: string) => (
    <div style={{ background: "#0f0f0f", border: `1px solid ${color}30`, borderRadius: "12px", padding: "20px" }}>
      <div style={{ fontSize: "11px", color: color, letterSpacing: "2px", textTransform: "uppercase", marginBottom: "12px", fontWeight: 700 }}>{label}</div>
      {items?.map((item: string, i: number) => (
        <div key={i} style={{ fontSize: "13px", color: "#888", marginBottom: "8px", paddingLeft: "12px", borderLeft: `2px solid ${color}` }}>{item}</div>
      ))}
    </div>
  );
    ...
    <main style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ maxWidth: "700px", margin: "0 auto", padding: "60px 40px 40px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "42px", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-1px", marginBottom: "16px" }}>
          SWOT Analysis <span style={{ color: "#7c3aed" }}>Generator</span>
        </h1>
        <p style={{ fontSize: "16px", color: "#888", lineHeight: 1.6, marginBottom: "40px", fontWeight: 300 }}>
          Analyse strengths, weaknesses, opportunities and threats in seconds.
        </p>
      </section>
      ...
      <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
          ...
          <input value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="Industry (e.g. SaaS, retail, fitness...)" style={inputStyle} />
          <input value={competitors} onChange={(e) => setCompetitors(e.target.value)} placeholder="Main competitors (e.g. Notion, Canva...)" style={inputStyle} />
          <input value={goals} onChange={(e) => setGoals(e.target.value)} placeholder="Business goals (e.g. reach 1000 customers in 12 months...)" style={inputStyle} />
        </div>
      ...
      {previewResult && !isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 20px" }}>
          <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "20px" }}>
            <div style={{ fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", color: "#444", marginBottom: "12px" }}>Preview — Strengths</div>
            {previewResult.strengths?.map((s: string, i: number) => (
              <div key={i} style={{ fontSize: "13px", color: "#888", marginBottom: "8px", paddingLeft: "12px", borderLeft: "2px solid #4ade80" }}>{s}</div>
            ))}
          </div>
        </section>
      )}
        ...
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
          <div style={{ position: "relative" }}>
            <div style={{ filter: "blur(6px)", pointerEvents: "none", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["Weaknesses", "Opportunities", "Threats"].map((t, i) => (
                <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "10px", padding: "16px 20px" }}>
                  ...
                  <div style={{ fontSize: "12px", color: "#555", marginTop: "4px" }}>4 detailed points included.</div>
                </div>
              ))}
            </div>
            <div style={{
              ...
              <div style={{
                ...
                <h3 style={{ fontSize: "20px", fontWeight: 800, marginBottom: "8px" }}>Unlock full SWOT analysis</h3>
      ...
      {fullResult && isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 80px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            {quadrant("Strengths", fullResult.strengths, "#4ade80")}
            {quadrant("Weaknesses", fullResult.weaknesses, "#f87171")}
            {quadrant("Opportunities", fullResult.opportunities, "#60a5fa")}
            {quadrant("Threats", fullResult.threats, "#fb923c")}
          </div>
        </section>
      )}
