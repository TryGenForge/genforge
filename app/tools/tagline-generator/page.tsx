...
export default function TaglineGenerator() {
  ...
  const [results, setResults] = useState<Tagline[]>([]);
  const [previewResult, setPreviewResult] = useState<Tagline | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/tagline", {
        ...
        body: JSON.stringify({ brandName, industry, tone }),
      });
      const data = await res.json();
      if (isPro) {
        setResults(data.taglines);
        setPreviewResult(null);
      } else {
        setPreviewResult(data.taglines[0]);
        setResults([]);
        setShowPaywall(true);
      }
    } catch (err) {
    ...
    <main style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ maxWidth: "700px", margin: "0 auto", padding: "60px 40px 40px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "42px", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-1px", marginBottom: "16px" }}>
          Tagline <span style={{ color: "#7c3aed" }}>Generator</span>
        </h1>
        <p style={{ fontSize: "16px", color: "#888", lineHeight: 1.6, marginBottom: "40px", fontWeight: 300 }}>
          Create catchy, memorable taglines that make your brand stick in people's minds.
        </p>
      </section>
      ...
      <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
          <input
            ...
            placeholder="Brand name (e.g. GenForge)"
            style={{
          ...
          <input
            ...
            placeholder="Industry (e.g. AI, fitness, coffee...)"
            style={{
              background: "#0f0f0f", border: "1px solid #242424",
              borderRadius: "10px", padding: "14px 18px", color: "#fff",
              fontSize: "14px", outline: "none", fontFamily: "inherit"
      ...
      {previewResult && !isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 20px" }}>
          <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "20px", marginBottom: "12px" }}>
            <div style={{ fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", color: "#444", marginBottom: "10px" }}>
              Preview
            </div>
            <div style={{ fontSize: "18px", fontWeight: 700, marginBottom: "8px" }}>
              "{previewResult.tagline}"
            </div>
      ...
      {/* BLURRED RESULTS (non-PRO) */}
      {showPaywall && !isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
          <div style={{ position: "relative" }}>
            <div style={{ filter: "blur(6px)", pointerEvents: "none", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["Unlock your brand's potential.", "Built for the bold.", "Where ideas become reality.", "Beyond the ordinary.", "Your vision, amplified."].map((t, i) => (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {results.map((item, i) => (
              <div key={i} style={{
                ...
                <button
                  onClick={() => handleCopy(item.tagline)}
                  style={{
                    background: "none", border: "1px solid #242424", borderRadius: "6px",
                    padding: "6px 10px", cursor: "pointer",
                    color: copied === item.tagline ? "#a78bfa" : "#444",
                    fontSize: "12px", flexShrink: 0, marginLeft: "12px", fontFamily: "inherit"
                  }}
                >
                  {copied === item.tagline ? "✓" : "Copy"}
                </button>
              </div>
            ))}
          </div>
