...
export default function SeoMetaGenerator() {
  ...
  const [previewResult, setPreviewResult] = useState<Meta | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/seo-meta", {
        ...
        body: JSON.stringify({ pageName, industry, keywords, audience }),
      });
    ...
    <main style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ maxWidth: "700px", margin: "0 auto", padding: "60px 40px 40px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "42px", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-1px", marginBottom: "16px" }}>
          SEO Meta <span style={{ color: "#7c3aed" }}>Generator</span>
        </h1>
        <p style={{ fontSize: "16px", color: "#888", lineHeight: 1.6, marginBottom: "40px", fontWeight: 300 }}>
          Generate optimised title tags and meta descriptions that rank on Google.
        </p>
      </section>

      <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
          <input value={pageName} onChange={(e) => setPageName(e.target.value)} placeholder="Page name (e.g. Homepage, About Us, Product Page...)" style={{ background: "#0f0f0f", border: "1px solid #242424", borderRadius: "10px", padding: "14px 18px", color: "#fff", fontSize: "14px", outline: "none", fontFamily: "inherit" }} />
          <input value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="Industry (e.g. AI, e-commerce, fitness...)" style={{ background: "#0f0f0f", border: "1px solid #242424", borderRadius: "10px", padding: "14px 18px", color: "#fff", fontSize: "14px", outline: "none", fontFamily: "inherit" }} />
          <input value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder="Target keywords (e.g. AI business tools, name generator...)" style={{ background: "#0f0f0f", border: "1px solid #242424", borderRadius: "10px", padding: "14px 18px", color: "#fff", fontSize: "14px", outline: "none", fontFamily: "inherit" }} />
          <input value={audience} onChange={(e) => setAudience(e.target.value)} placeholder="Target audience (e.g. entrepreneurs, developers...)" style={{ background: "#0f0f0f", border: "1px solid #242424", borderRadius: "10px", padding: "14px 18px", color: "#fff", fontSize: "14px", outline: "none", fontFamily: "inherit" }} />
        </div>
        <button onClick={handleGenerate} disabled={loading || !pageName.trim()} style={{ width: "100%", background: loading ? "#222" : "#7c3aed", color: "#fff", border: "none", borderRadius: "10px", padding: "14px", fontSize: "15px", fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
          {loading ? "Generating..." : "Generate SEO Meta Tags"}
        </button>
      </section>
        ...
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 20px" }}>
          <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "20px" }}>
            ...
            <div style={{ marginBottom: "8px" }}>
              ...
              <div style={{ fontSize: "15px", fontWeight: 700, color: "#a78bfa" }}>{previewResult.title}</div>
            </div>
            <div style={{ marginBottom: "10px" }}>
              <div style={{ fontSize: "11px", color: "#666", marginBottom: "4px" }}>DESCRIPTION</div>
              <div style={{ fontSize: "13px", lineHeight: 1.6 }}>{previewResult.description}</div>
            </div>
        ...
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
          <div style={{ position: "relative" }}>
            <div style={{ filter: "blur(6px)", pointerEvents: "none", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["AI Tools for Business | Grow Faster", "Best Business Name Generator | Free AI Tool", "GenForge — AI Business Tools | One Payment", "Start Your Business with AI | GenForge", "Free AI Tools for Entrepreneurs | GenForge"].map((t, i) => (
                <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "10px", padding: "16px 20px" }}>
                  <div style={{ fontSize: "14px", fontWeight: 700, color: "#a78bfa" }}>{t}</div>
                </div>
              ))}
            </div>
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "linear-gradient(to bottom, transparent, #080808 60%)" }}>
              <div style={{ background: "linear-gradient(135deg, #130d20, #0e0e16)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: "14px", padding: "28px 32px", textAlign: "center", maxWidth: "340px" }}>
                <h3 style={{ fontSize: "20px", fontWeight: 800, marginBottom: "8px" }}>Unlock all 6 meta sets</h3>
            ...
            {results.map((item, i) => (
              <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "18px 20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "10px" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: "11px", color: "#666", marginBottom: "4px" }}>TITLE</div>
                    <div style={{ fontSize: "15px", fontWeight: 700, color: "#a78bfa", marginBottom: "8px" }}>{item.title}</div>
                    <div style={{ fontSize: "11px", color: "#666", marginBottom: "4px" }}>DESCRIPTION</div>
                    <div style={{ fontSize: "13px", lineHeight: 1.5, marginBottom: "8px" }}>{item.description}</div>
                    <div style={{ fontSize: "12px", color: "#555" }}>{item.reason}</div>
                  </div>
                  <button onClick={() => handleCopy(`${item.title}\n${item.description}`)} style={{ background: "none", border: "1px solid #242424", borderRadius: "6px", padding: "6px 10px", cursor: "pointer", color: copied === `${item.title}\n${item.description}` ? "#a78bfa" : "#444", fontSize: "12px", flexShrink: 0, marginLeft: "12px", fontFamily: "inherit" }}>
