...
export default function ElevatorPitchGenerator() {
  ...
  const [previewResult, setPreviewResult] = useState<Pitch | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/elevator-pitch", {
        ...
        body: JSON.stringify({ companyName, industry, problem, solution }),
      });
      const data = await res.json();
      if (isPro) {
        setResults(data.pitches);
        setPreviewResult(null);
      } else {
        setPreviewResult(data.pitches[0]);
        setResults([]);
        setShowPaywall(true);
      }
    } catch (err) {
    ...
    <main style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ maxWidth: "700px", margin: "0 auto", padding: "60px 40px 40px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "42px", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-1px", marginBottom: "16px" }}>
          Elevator Pitch <span style={{ color: "#7c3aed" }}>Generator</span>
        </h1>
        <p style={{ fontSize: "16px", color: "#888", lineHeight: 1.6, marginBottom: "40px", fontWeight: 300 }}>
          Craft the perfect 30-second pitch for investors, customers, and partners.
        </p>
      </section>
      ...
      <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
          ...
          <input value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="Industry (e.g. AI, fintech, health...)" style={{ background: "#0f0f0f", border: "1px solid #242424", borderRadius: "10px", padding: "14px 18px", color: "#fff", fontSize: "14px", outline: "none", fontFamily: "inherit" }} />
          <input value={problem} onChange={(e) => setProblem(e.target.value)} placeholder="Problem you solve (e.g. businesses waste hours on branding)" style={{ background: "#0f0f0f", border: "1px solid #242424", borderRadius: "10px", padding: "14px 18px", color: "#fff", fontSize: "14px", outline: "none", fontFamily: "inherit" }} />
          <input value={solution} onChange={(e) => setSolution(e.target.value)} placeholder="Your solution (e.g. AI tools that generate brand assets instantly)" style={{ background: "#0f0f0f", border: "1px solid #242424", borderRadius: "10px", padding: "14px 18px", color: "#fff", fontSize: "14px", outline: "none", fontFamily: "inherit" }} />
        </div>
        <button onClick={handleGenerate} disabled={loading || !companyName.trim()} style={{ width: "100%", background: loading ? "#222" : "#7c3aed", color: "#fff", border: "none", borderRadius: "10px", padding: "14px", fontSize: "15px", fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
          {loading ? "Generating..." : "Generate Elevator Pitches"}
        </button>
      </section>
      ...
      {previewResult && !isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 20px" }}>
          <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "20px" }}>
            ...
            <div style={{ fontSize: "15px", lineHeight: 1.7, marginBottom: "8px" }}>{previewResult.pitch}</div>
        ...
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
          <div style={{ position: "relative" }}>
            <div style={{ filter: "blur(6px)", pointerEvents: "none", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["Every business deserves great branding.", "We built this so you don't have to.", "The fastest way from idea to brand.", "AI that works while you sleep.", "Your unfair advantage starts here."].map((t, i) => (
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "linear-gradient(to bottom, transparent, #080808 60%)" }}>
              <div style={{ background: "linear-gradient(135deg, #130d20, #0e0e16)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: "14px", padding: "28px 32px", textAlign: "center", maxWidth: "340px" }}>
                <h3 style={{ fontSize: "20px", fontWeight: 800, marginBottom: "8px" }}>Unlock all 6 pitches</h3>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {results.map((item, i) => (
              <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "18px 20px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: "15px", lineHeight: 1.7, marginBottom: "6px" }}>{item.pitch}</div>
                  <div style={{ fontSize: "12px", color: "#666" }}>{item.reason}</div>
                </div>
                <button onClick={() => handleCopy(item.pitch)} style={{ background: "none", border: "1px solid #242424", borderRadius: "6px", padding: "6px 10px", cursor: "pointer", color: copied === item.pitch ? "#a78bfa" : "#444", fontSize: "12px", flexShrink: 0, marginLeft: "12px", fontFamily: "inherit" }}>
                  {copied === item.pitch ? "✓" : "Copy"}
                </button>
              </div>
            ))}
          </div>
