...
export default function LandingPageCopyGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/landing-page-copy", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ businessName, product, audience, benefit }),
      });
  ...
  const handleCopy = () => {
    ...
    const text = `${fullResult.headline}\n\n${fullResult.subheadline}\n\n${fullResult.heroDescription}\n\nFeatures:\n${fullResult.features?.map((f: any) => `${f.title}: ${f.description}`).join("\n")}\n\n${fullResult.socialProof}\n\n${fullResult.cta}\n${fullResult.ctaDescription}`;
    ...
    <main style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ maxWidth: "700px", margin: "0 auto", padding: "60px 40px 40px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "42px", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-1px", marginBottom: "16px" }}>
          Landing Page Copy <span style={{ color: "#7c3aed" }}>Generator</span>
        </h1>
        <p style={{ fontSize: "16px", color: "#888", lineHeight: 1.6, marginBottom: "40px", fontWeight: 300 }}>
          Full landing page copy ready to publish in seconds.
        </p>
      </section>
      ...
      <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
          ...
          <input value={product} onChange={(e) => setProduct(e.target.value)} placeholder="Product or service (e.g. AI tools subscription...)" style={inputStyle} />
          ...
          <input value={benefit} onChange={(e) => setBenefit(e.target.value)} placeholder="Main benefit (e.g. save 10 hours a week...)" style={inputStyle} />
        </div>
      ...
      {previewResult && !isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 20px" }}>
          <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
            <div style={{ fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", color: "#444", marginBottom: "10px" }}>Preview — Hero Section</div>
            ...
            <div style={{ fontSize: "13px", color: "#888", lineHeight: 1.7 }}>{previewResult.heroDescription}</div>
          </div>
        </section>
      )}
        ...
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
          <div style={{ position: "relative" }}>
            <div style={{ filter: "blur(6px)", pointerEvents: "none", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["Features section (3 features)", "Social proof & testimonial", "CTA section with button copy"].map((t, i) => (
            ...
            <div style={{
              ...
              <div style={{
                ...
                <h3 style={{ fontSize: "20px", fontWeight: 800, marginBottom: "8px" }}>Unlock full landing page copy</h3>
      ...
      {fullResult && isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 80px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
              <div style={{ fontSize: "11px", color: "#444", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "10px" }}>Hero Section</div>
              <div style={{ fontSize: "22px", fontWeight: 800, marginBottom: "8px" }}>{fullResult.headline}</div>
              <div style={{ fontSize: "15px", color: "#666", marginBottom: "12px" }}>{fullResult.subheadline}</div>
              <div style={{ fontSize: "13px", color: "#888", lineHeight: 1.7 }}>{fullResult.heroDescription}</div>
            </div>
            <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
              <div style={{ fontSize: "11px", color: "#444", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "16px" }}>Features</div>
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                {fullResult.features?.map((f: any, i: number) => (
                  <div key={i} style={{ paddingLeft: "12px", borderLeft: "2px solid #7c3aed" }}>
                    <div style={{ fontSize: "14px", fontWeight: 700, marginBottom: "4px" }}>{f.title}</div>
                    <div style={{ fontSize: "13px", color: "#888" }}>{f.description}</div>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
              <div style={{ fontSize: "11px", color: "#444", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "10px" }}>Social Proof</div>
              <div style={{ fontSize: "14px", color: "#888", fontStyle: "italic", lineHeight: 1.7 }}>"{fullResult.socialProof}"</div>
            </div>
            <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px", textAlign: "center" }}>
              <div style={{ fontSize: "11px", color: "#444", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "16px" }}>Call to Action</div>
              <div style={{ background: "#7c3aed", color: "#fff", padding: "12px 28px", borderRadius: "8px", fontSize: "15px", fontWeight: 700, display: "inline-block", marginBottom: "10px" }}>{fullResult.cta}</div>
              <div style={{ fontSize: "12px", color: "#555" }}>{fullResult.ctaDescription}</div>
            </div>
          </div>
        </section>
      )}
