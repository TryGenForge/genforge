...
export default function BusinessPlanGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/business-plan", {
        ...
        body: JSON.stringify({ businessName, industry, targetMarket, revenue, timeframe }),
      });
  ...
  const handleCopy = () => {
    ...
    const text = `EXECUTIVE SUMMARY\n${fullResult.executive}\n\nPROBLEM\n${fullResult.problem}\n\nSOLUTION\n${fullResult.solution}\n\nMARKET\n${fullResult.market}\n\nBUSINESS MODEL\n${fullResult.model}\n\nMILESTONES\n${fullResult.milestones?.join("\n")}\n\nRISKS\n${fullResult.risks?.join("\n")}`;
    ...
    <main style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ maxWidth: "700px", margin: "0 auto", padding: "60px 40px 40px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "42px", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-1px", marginBottom: "16px" }}>
          Business Plan <span style={{ color: "#7c3aed" }}>Generator</span>
        </h1>
        <p style={{ fontSize: "16px", color: "#888", lineHeight: 1.6, marginBottom: "40px", fontWeight: 300 }}>
          A concise business plan you can share with investors instantly.
        </p>
      </section>
      ...
      <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
          ...
          <input value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="Industry (e.g. SaaS, e-commerce, food...)" style={inputStyle} />
          <input value={targetMarket} onChange={(e) => setTargetMarket(e.target.value)} placeholder="Target market (e.g. small business owners in the UK...)" style={inputStyle} />
          <input value={revenue} onChange={(e) => setRevenue(e.target.value)} placeholder="Revenue model (e.g. one-time payment, subscription...)" style={inputStyle} />
      ...
      {previewResult && !isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 20px" }}>
          <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
            <div style={{ fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", color: "#444", marginBottom: "10px" }}>Preview — Executive Summary</div>
            <div style={{ fontSize: "13px", color: "#ccc", lineHeight: 1.8 }}>{previewResult.executive}</div>
          </div>
        </section>
      )}
        ...
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
          <div style={{ position: "relative" }}>
            <div style={{ filter: "blur(6px)", pointerEvents: "none", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["Problem & Solution", "Market Analysis", "Business Model", "Milestones & Risks"].map((t, i) => (
                <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "10px", padding: "16px 20px" }}>
                  ...
                  <div style={{ fontSize: "12px", color: "#555", marginTop: "4px" }}>Full section included.</div>
                </div>
              ))}
            </div>
            <div style={{
              ...
              <div style={{
                ...
                <h3 style={{ fontSize: "20px", fontWeight: 800, marginBottom: "8px" }}>Unlock full business plan</h3>
      ...
      {fullResult && isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 80px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            {[
              { label: "Executive Summary", content: fullResult.executive },
              { label: "Problem", content: fullResult.problem },
              { label: "Solution", content: fullResult.solution },
              { label: "Market Opportunity", content: fullResult.market },
              { label: "Business Model", content: fullResult.model },
            ].map((s, i) => (
              <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
                <div style={{ fontSize: "11px", color: "#444", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "10px" }}>{s.label}</div>
                <div style={{ fontSize: "13px", color: "#ccc", lineHeight: 1.8 }}>{s.content}</div>
              </div>
            ))}
            <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
              <div style={{ fontSize: "11px", color: "#444", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "12px" }}>Milestones</div>
              {fullResult.milestones?.map((m: string, i: number) => (
                <div key={i} style={{ fontSize: "13px", color: "#888", marginBottom: "8px", paddingLeft: "12px", borderLeft: "2px solid #7c3aed" }}>{m}</div>
              ))}
            </div>
            <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
              <div style={{ fontSize: "11px", color: "#f87171", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "12px" }}>Risks</div>
              {fullResult.risks?.map((r: string, i: number) => (
                <div key={i} style={{ fontSize: "13px", color: "#888", marginBottom: "8px", paddingLeft: "12px", borderLeft: "2px solid #f87171" }}>{r}</div>
              ))}
            </div>
          </div>
        </section>
