...
export default function TestimonialRequestGenerator() {
  ...
  const [fullResult, setFullResult] = useState<Email[] | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/testimonial-request", {
        ...
        body: JSON.stringify({ businessName, product, customerName, platform }),
      });
      const data = await res.json();
      if (isPro) {
        setFullResult(data.emails);
    ...
    <main style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ maxWidth: "700px", margin: "0 auto", padding: "60px 40px 40px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "42px", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-1px", marginBottom: "16px" }}>
          Testimonial Request <span style={{ color: "#7c3aed" }}>Generator</span>
        </h1>
        <p style={{ fontSize: "16px", color: "#888", lineHeight: 1.6, marginBottom: "40px", fontWeight: 300 }}>
          Emails that get customers to leave reviews effortlessly.
        </p>
      </section>
      ...
      <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px" }}>
          ...
          <input value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Customer name (e.g. John, or leave blank for generic)" style={inputStyle} />
        ...
        <button onClick={handleGenerate} disabled={loading || !businessName.trim()} style={{
          ...
          {loading ? "Generating..." : "Generate Request Emails"}
        </button>
      </section>
      ...
      {previewResult && !isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 20px" }}>
          <div style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
            ...
            <div style={{ fontSize: "12px", color: "#444", marginBottom: "6px" }}>Subject:</div>
            <div style={{ fontSize: "14px", fontWeight: 700, marginBottom: "16px" }}>{previewResult.subject}</div>
            <div style={{ fontSize: "13px", color: "#888", lineHeight: 1.8, whiteSpace: "pre-line" }}>{previewResult.body.slice(0, 200)}...</div>
          </div>
        </section>
      )}
        ...
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 40px" }}>
          <div style={{ position: "relative" }}>
            <div style={{ filter: "blur(6px)", pointerEvents: "none", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["Email variation 2 — Different style", "Email variation 3 — Different angle"].map((t, i) => (
                <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "10px", padding: "16px 20px" }}>
                  ...
                  <div style={{ fontSize: "12px", color: "#555", marginTop: "4px" }}>Full email ready to send.</div>
                </div>
              ))}
            </div>
            <div style={{
              ...
              <div style={{
                ...
                <h3 style={{ fontSize: "20px", fontWeight: 800, marginBottom: "8px" }}>Unlock all 3 email styles</h3>
      ...
      {fullResult && isPro && (
        <section style={{ maxWidth: "600px", margin: "0 auto", padding: "0 40px 80px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            {fullResult.map((email, i) => (
              <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: "12px", padding: "24px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
                  <div style={{ display: "inline-block", background: "rgba(124,58,237,0.12)", color: "#a78bfa", border: "1px solid rgba(124,58,237,0.25)", padding: "3px 10px", borderRadius: "20px", fontSize: "11px", fontWeight: 600 }}>{email.style}</div>
                  <button onClick={() => handleCopy(`Subject: ${email.subject}\n\n${email.body}`)} style={{
                    background: "none", border: "1px solid #242424", borderRadius: "6px",
                    padding: "6px 10px", cursor: "pointer",
                    color: copied === `Subject: ${email.subject}\n\n${email.body}` ? "#a78bfa" : "#444",
                    fontSize: "12px", fontFamily: "inherit"
                  }}>
                    {copied === `Subject: ${email.subject}\n\n${email.body}` ? "✓ Copied" : "Copy"}
                  </button>
                </div>
                <div style={{ fontSize: "11px", color: "#444", marginBottom: "4px" }}>Subject:</div>
                <div style={{ fontSize: "14px", fontWeight: 700, marginBottom: "16px" }}>{email.subject}</div>
                <div style={{ fontSize: "13px", color: "#888", lineHeight: 1.8, whiteSpace: "pre-line" }}>{email.body}</div>
              </div>
            ))}
          </div>
        </section>
